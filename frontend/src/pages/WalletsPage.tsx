...
export function WalletsPage() {
  ...
  return <div className="page-content"><PageHeading title="My Wallets" description="All your deposit addresses across supported chains." />{error && <Notice variant="danger">{error}</Notice>}{loading ? <LoadingPanel /> : <Card className="panel-card wallet-page-card"><div className="wallet-list">{wallets.map((wallet) => <WalletRow key={wallet.id} wallet={wallet} onReceive={() => openAction('receive', wallet.symbol)} />)}</div></Card>}</div>
}
