...
export function ActionModal({
  ...
  return (
    <Modal title={title} onClose={onClose}>
      <div className="modal-body space-y-5">
        {step === 'choose' && <><p className="modal-copy">Choose a simulated transfer method.</p><button className="method-card" onClick={() => setStep('crypto')}><span className="method-icon red"><WalletCards /></span><span><strong>To a crypto wallet</strong><small>Uses the profile confirmation-code workflow.</small></span><ArrowRight size={19} /></button><button className="method-card selected" onClick={() => setStep('card')}><span className="method-icon blue"><CreditCard /></span><span><strong>To a bank card</strong><small>Only the final four digits are stored.</small></span><ArrowRight size={19} /></button><button className="method-card" disabled><span className="method-icon green"><Landmark /></span><span><strong>Cash pickup</strong><small>Not available in this demo.</small></span><ShieldCheck size={17} /></button></>}
        {step === 'crypto' && <form className="space-y-4" onSubmit={submitCrypto}><AssetAndAmount wallets={wallets} symbol={symbol} setSymbol={setSymbol} amount={amount} setAmount={setAmount} /><label className="field-label">Destination address<input className="field-input" value={address} onChange={(event) => setAddress(event.target.value)} placeholder="Wallet address" required /></label><ErrorNotice message={error} /><div className="modal-actions"><button type="button" className="button secondary" onClick={() => setStep('choose')}><ArrowLeft size={17} /> Back</button><button className="button primary" disabled={busy}>{busy ? 'Preparing…' : 'Continue'}</button></div></form>}
        {step === 'card' && <form className="space-y-4" onSubmit={submitCard}><AssetAndAmount wallets={wallets} symbol={symbol} setSymbol={setSymbol} amount={amount} setAmount={setAmount} /><label className="field-label">Demo card number<input className="field-input font-mono tracking-[0.18em]" value={cardNumber} onChange={(event) => setCardNumber(event.target.value)} inputMode="numeric" required /><small>Only the last four digits are saved.</small></label><ErrorNotice message={error} /><div className="modal-actions"><button type="button" className="button secondary" onClick={() => setStep('choose')}><ArrowLeft size={17} /> Back</button><button className="button primary" disabled={busy}>{busy ? 'Preparing…' : 'Continue'}</button></div></form>}
        {step === 'verify' && transfer && <form className="space-y-4" onSubmit={submitCode}><div className="info-panel"><ShieldCheck /><div><strong>Confirmation in progress</strong><p>Enter the next one-time code supplied for this demo profile.</p></div></div><div className="verification-progress"><span>{transfer.used_codes} of {transfer.required_codes} codes entered</span><progress value={transfer.used_codes} max={Math.max(1, transfer.required_codes)} /></div><label className="field-label">Next confirmation code<input className="field-input otp-input" value={verificationCode} onChange={(event) => setVerificationCode(event.target.value.replace(/\D/g, '').slice(0, 6))} inputMode="numeric" maxLength={6} autoFocus required /></label><ErrorNotice message={error} /><button className="button primary w-full" disabled={busy || verificationCode.length !== 6}>{busy ? 'Checking…' : 'Submit code'}</button></form>}
        {step === 'processing' && transfer && <div className="pending-state"><span className="pending-spinner"><RefreshCw /></span><h3>Demo transfer is processing</h3><p>The saved processing window ends in <strong>{remainingTime(transfer.processing_until)}</strong>. You can close this window; progress will be restored later.</p><button className="button primary w-full" onClick={onClose}>Close</button></div>}
        {step === 'completed' && <Completion message="Demo transfer completed" onClose={onClose} />}
      </div>
    </Modal>
  )
}
...
function AssetAndAmount({ wallets, symbol, setSymbol, amount, setAmount }: { wallets: Wallet[]; symbol: string; setSymbol: (value: string) => void; amount: string; setAmount: (value: string) => void }) {
  ...
  return <><label className="field-label">Coin<CustomSelect ariaLabel="Coin" value={symbol} onChange={setSymbol} options={wallets.map((item) => ({ value: item.symbol, label: `${item.name} (${item.symbol})` }))} /></label><label className="field-label">Amount<input className="field-input" type="number" min="0.00000001" step="any" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0.00" required />{wallet && <small>Available: {assetAmount(wallet.balance, wallet.symbol)}</small>}</label></>
}

function Completion({ message, onClose }: { message: string; onClose: () => void }) {
  return <div className="pending-state"><span className="completion-mark"><Check /></span><h3>{message}</h3><p>The transaction status is saved in your history.</p><button className="button primary w-full" onClick={onClose}>Done</button></div>
}
...
function BuyModalContent({ wallets, symbol, setSymbol, onClose, changed }: { wallets: Wallet[]; symbol: string; setSymbol: (value: string) => void; onClose: () => void; changed: () => void }) {
  ...
  return <Modal title="Buy crypto" onClose={onClose}><div className="modal-body space-y-5">{done ? <Completion message={done} onClose={onClose} /> : <form className="space-y-4" onSubmit={submit}><div className="asset-highlight"><span className="method-icon red"><BadgeDollarSign /></span><div><strong>Instant demo purchase</strong><span>Credits the simulated balance.</span></div></div><label className="field-label">Coin<CustomSelect ariaLabel="Coin" value={symbol} onChange={setSymbol} options={wallets.map((item) => ({ value: item.symbol, label: `${item.name} (${item.symbol})` }))} /></label><label className="field-label">Amount in USD<input className="field-input" type="number" min="1" max="50000" value={amount} onChange={(event) => setAmount(event.target.value)} required /></label><ErrorNotice message={error} /><button className="button primary w-full" disabled={busy}>{busy ? 'Buying…' : 'Buy'}</button></form>}</div></Modal>
}
