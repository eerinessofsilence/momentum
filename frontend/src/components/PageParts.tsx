...
export function WalletRow({ wallet, compact = false, onReceive }: { wallet: Wallet; compact?: boolean; onReceive?: () => void }) {
  ...
  return (
    <div className={`wallet-row ${compact ? 'compact' : ''}`}>
      ...
      {compact && <span className={`change-chip ${positive ? 'positive' : 'negative'}`}>{positive ? '+' : ''}{wallet.change_24h}%</span>}
      {!compact && onReceive && <button className="button secondary small" onClick={onReceive}>Receive</button>}
    </div>
  )
}
...
export function TransactionRow({ transaction, compact = false }: { transaction: Transaction; compact?: boolean }) {
  ...
  return (
    <div className={`transaction-row ${compact ? 'compact' : ''}`}>
      <span className={`transaction-icon ${incoming ? 'incoming' : 'outgoing'}`}><Icon size={19} /></span>
      <div className="transaction-title"><strong>{transaction.title}</strong><span className={`status-chip ${transaction.status}`}>{transaction.status}</span>{!compact && <small>{new Date(transaction.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</small>}</div>
